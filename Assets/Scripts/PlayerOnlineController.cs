...
public class PlayerOnlineController : Photon.MonoBehaviour, IPunObservable
{
    ...
    public PointAtObject armPointer;
    ...
    void IPunObservable.OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.isWriting)
        {
            ...
            stream.SendNext(cursorObj);
            stream.SendNext(armObj);
            stream.SendNext(armPointer);
        }
        else
        {
            currHealth = (float)stream.ReceiveNext();
            cursorObj = (GameObject)stream.ReceiveNext();
            armObj = (GameObject)stream.ReceiveNext();
            armPointer = (PointAtObject)stream.ReceiveNext();
        }
    }
}
