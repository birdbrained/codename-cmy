...
public class LampBoss : Boss
{
    ...
    [SerializeField]
    private float attackSustainTime = 4.0f;
    public float normalDamage;
    private bool startedLaserAttack = false;

    private RotateBetweenTwoAngles rotator;
    private Bullet laserBullet;
    ...
    public override void Start()
    {
        ...
        rotator = GetComponentInChildren<RotateBetweenTwoAngles>();
        if (healthBar != null)
        {
        ...
        if (laserObj != null)
        {
            laserBullet = laserObj.GetComponent<Bullet>();
            laserMaterial = laserObj.GetComponent<Renderer>().material;
    ...
    public void Attack()
    {
        if (!startedLaserAttack)
        {
            startedLaserAttack = true;
            laserBullet.SetBulletAttributes(gameObject, GetCurrentColor(), GetCurrentColorIndex(), normalDamage);
            laserMaterial.color = GetCurrentColor();
    ...
    {
        yield return new WaitForSeconds(attackSustainTime);
        MyAnimator.SetTrigger("attackFinish");
        startedLaserAttack = false;
