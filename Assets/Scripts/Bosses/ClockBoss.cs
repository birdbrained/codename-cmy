...
public class ClockBoss : Boss
{
    ...
    [SerializeField]
    private float attackDelayTime = 5.0f;
    private float attackTimer = 0.0f;
    [SerializeField]
    private float attackSustainTime = 5.0f;
    [SerializeField]
    ...
    public override void Start()
    {
        ...
        rotators = GetComponentsInChildren<Rotate>();
        if (healthBar != null)
        {
    ...
    void FixedUpdate()
    {
        ...
        if (!Attacking)
        {
            ...
            if (attackTimer >= attackDelayTime)
            {
                ...
                StartCoroutine(WaitForAttackToFinish(attackSustainTime));
            }
        }
        else
        {
        ...
        if (IsDead)
        {
            foreach (Rotate rot in rotators)
            {
                rot.canRotate = false;
            }
            if (deathTimer > 0.0f)
    ...
    public void Attack()
    {
        GameObject _bullet = Instantiate(bulletObj, bulletSpawnPoint.position, minuteHand.transform.rotation);
        Bullet _bulletComponent = _bullet.GetComponent<Bullet>();
        _bullet.tag = "enemy_laser";
        _bulletComponent.SetBulletAttributes(gameObject, GetCurrentColor(), GetCurrentColorIndex(), normalDamage);
    }

    IEnumerator WaitForAttackToFinish(float f)
    {
