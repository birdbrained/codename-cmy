...
public class PrinterBossOnline : Boss, IPunObservable
{
    ...
    [SerializeField]
    private string bulletObj;
    [SerializeField]
    private string chargeBulletObj;
    ...
    public void Attack(int side)
    {
        ...
        {
            GameObject _bullet;
            BulletOnline _bulletComponent;

            if (side == 0)
            {
                _bullet = PhotonNetwork.Instantiate(bulletObj, leftFireSpawn.transform.position, Quaternion.Euler(new Vector3(0, 0, -45 + Random.Range(-angleOffset, angleOffset))), 0);

                _bulletComponent = _bullet.GetComponent<BulletOnline>();
            ...
            {
                _bullet = PhotonNetwork.Instantiate(bulletObj, rightFireSpawn.transform.position, Quaternion.Euler(new Vector3(0, 0, 45 + Random.Range(-angleOffset, angleOffset))), 0);
                _bulletComponent = _bullet.GetComponent<BulletOnline>();
                _bullet.tag = "enemy_bullet";
                _bullet.transform.localScale *= 2;
                /*_bulletComponent.FireSprite.material.color = currColors[currColorEquipped];
    ...
    public void ChargeAttack()
    {
        ...
        _bullet = PhotonNetwork.Instantiate(chargeBulletObj, centerFireSpawn.transform.position, transform.rotation, 0);
        _bulletComponent = _bullet.GetComponent<BulletOnline>();
        _bullet.tag = "enemy_bullet";
        _bullet.transform.localScale *= 4.0f;
        /*_bulletComponent.FireSprite.material.color = currColors[currColorEquipped];
		_bulletComponent.colorIndex = currColorIndexes[currColorEquipped];
